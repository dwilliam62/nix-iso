...
echo -e "${BLUE}Cloning ZaneyOS from GitLab (branch: main)...${NC}"
if [ -d "/mnt/etc/nixos/zaneyos" ]; then
  echo -e "${YELLOW}Removing existing ZaneyOS installation${NC}"
  rm -rf /mnt/etc/nixos/zaneyos
fi
git clone https://gitlab.com/zaney/zaneyos.git -b main --depth=1 /mnt/etc/nixos/zaneyos
...
for file in hosts/default/*.nix; do
  ...
  if [ "$filename" != "hardware.nix" ]; then
    cp "$file" hosts/"$hostName"/
  fi
done
...
HOME=/root nixos-install --flake /mnt/etc/nixos/zaneyos#${profile} --option accept-flake-config true --no-write-lock-file --no-root-passwd
