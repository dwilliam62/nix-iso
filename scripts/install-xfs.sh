#!/usr/bin/env bash
# Interactive installer for an XFS-based NixOS system.
# - Prompts for timezone, keymap, hostname, username
# - Lets user select target disk
# - Partitions (GPT: 1GiB ESP + rest XFS)
...
for dep in lsblk parted mkfs.fat mkfs.xfs mount umount sed awk tee nixos-generate-config nixos-install wipefs; do
  require "$dep"
done
...
${HASH_LINE:+${HASH_LINE}}
  ...
  environment.systemPackages = with pkgs; [
    git ncftp htop btop pciutils xfsprogs wget curl
